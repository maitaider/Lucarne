-- Sprint 0 — C-5 : points fantômes sur match `finished` sans score.
--
-- compute_bet_points ne contrôlait que v_match.status = 'finished', jamais la
-- non-nullité des scores. Avec home_score/away_score NULL :
--   - match_winner : NULL > NULL et NULL < NULL valent NULL → branche `else`
--     → v_actual_winner = 'draw' → un prono "nul" gagne +3 ;
--   - total_goals  : coalesce(home,0)+coalesce(away,0) = 0 → un prono "0" gagne
--     +5, un prono "1" gagne +2.
-- L'admin peut enregistrer status='finished' avec scores NULL, donc le risque
-- est réel. On ajoute un garde : si un des deux scores est NULL, on ne settle
-- PAS le pari (il reste 'validated' et se réglera au prochain recompute, une
-- fois les scores saisis). Couvre TOUS les chemins (admin_set_match_result,
-- admin_recompute_match, cron_sync_match) car tous passent par cette fonction.
-- L'UI (match-results-admin.tsx) ajoute en complément un garde côté client.
--
-- Seule ligne ajoutée vs la définition live : le `if ... is null ... return`.
...
begin
  ...
  if v_bet.match_id is not null then
    ...
    -- C-5: a finished match with missing score(s) must not settle bets, or the
    -- coalesce/NULL-comparison logic below awards phantom points. Leave the bet
    -- 'validated'; it settles on the next recompute once the scores are entered.
