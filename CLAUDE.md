...
**Phase 3 (engagement & qualité) — entamée (itératif)** : ✅ tranche qualité — perf `React.cache(listMatches)` (déduplique les ~7 appels layout+pages par requête), `loading.tsx` (squelette app), `not-found.tsx` localisé, a11y formulaire support (`htmlFor`/`id`). ✅ **Pointage simplifié « score-only »** (branche `feat/bracket-scoring`, **PR #21**, migrations `20260603100000_knockout_penalties` + `20260603100100_score_only_scoring`) — **décision produit de l'humain (2026-06-04)** : le pointage est réduit à ce qui se déduit du **score final** d'un match : bon vainqueur +3, bon total de buts +5/+2, score exact +5 (cumulatif → max +13/match). **Retiré : tout le système de buteurs** (`anytime_scorer` +4 / `first_scorer` +8) côté joueur (predict-board/per-match-picker, quick-bet sheet), **côté admin** (plus de saisie de buteurs — l'admin n'entre que score + statut [+ tirs au but en phase finale]) et **affichage** (timeline de buts + buteurs sur la fiche match). `compute_bet_points` ne garde que match_winner/total_goals/exact_score. **Bracket NON scoré** : le scoring du bracket (essayé, testé, puis retiré sur décision de l'humain « trop compliqué/risque d'erreur/gestion ») n'est PAS livré ; la page bracket (`/predict` phase finale) reste un outil de planification **décoratif** (comme avant). Conservés (corrections de bugs, validées par l'humain) : **I-3** `admin_set_match_result(…, p_home_pen, p_away_pen)` résout `winner_team_id` des nuls KO aux tirs au but (+ UI admin + préremplissage anti-effacement) ; **I-2** confrontation directe FIFA dans `group-standings.ts` (affichage des classements de groupe). `picks-board`/`pick-row` = code mort (non monté, legacy `/picks`→redirect). Vérifié : build OK, typecheck OK, 46/46 tests, scoring testé psql (vainqueur 3/total 5/exact 13/buteurs 0). **PAS encore en prod** (`db push` + merge en attente du feu vert humain). Reste (XL, sessions dédiées) : profils publics `/u/[username]` + `@username` cliquables, notifs sociales, classement par phase, partage de prono, i18n → next-intl, design-system (radii, `<Button>`/`<Field>`), distribution cagnotte assistée.
