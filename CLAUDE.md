...
**Phase 3 (engagement & qualité) — entamée (itératif)** : ✅ tranche qualité — perf `React.cache(listMatches)` (déduplique les ~7 appels layout+pages par requête), `loading.tsx` (squelette app), `not-found.tsx` localisé, a11y formulaire support (`htmlFor`/`id`). Reste (XL, sessions dédiées) : profils publics `/u/[username]` + `@username` cliquables, notifs sociales, classement par phase, partage de prono, timeline de buts / compos, i18n → next-intl, design-system (radii, `<Button>`/`<Field>`), distribution cagnotte assistée.
